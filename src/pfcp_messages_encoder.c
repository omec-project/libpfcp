    ...
    uint8_t *buf)
{
	...
	((pfcp_header_t *) buf)->message_len = htons(enc_len - PFCP_IE_HDR_SIZE);
    ...
    uint8_t *buf)
{
    ...
    for (uint8_t i = 0; i < value->dnlnk_data_svc_info_count; i++) {
        if (value->dnlnk_data_svc_info[i].header.len)
            enc_len += encode_pfcp_dnlnk_data_svc_info_ie_t(&(value->dnlnk_data_svc_info[i]), buf + enc_len);
    }
...
/* TODO: Revisit this for change in yang */
	...
	if (value->src_intrc_type.header.len)
		enc_len += encode_pfcp_3gpp_intfc_type_ie_t(&(value->src_intrc_type), buf + enc_len);
		...
		uint8_t *buf)
{
	...
	for (uint8_t i = 0; i < value->created_pdr_count; i++) {
		if (value->created_pdr[i].header.len)
			enc_len += encode_pfcp_created_pdr_ie_t(&(value->created_pdr[i]), buf + enc_len);
	}
	...
	for (uint8_t i = 0; i < value->created_traffic_endpt_count; i++) {
		if (value->created_traffic_endpt[i].header.len)
			enc_len += encode_pfcp_created_traffic_endpt_ie_t(&(value->created_traffic_endpt[i]), buf + enc_len);
	}
