      ...
      pfcp_sess_rpt_req_t *value)
{
      ...
      return count + ((value->header.s == 1) ? sizeof(pfcp_header_t) : (sizeof(pfcp_header_t) - 8));
}
/**
      ...
      pfcp_dnlnk_data_rpt_ie_t *value)
{
      ...
      if (ie_type == PFCP_IE_PDR_ID) {
            ...
            count += decode_pfcp_dnlnk_data_svc_info_ie_t(buf + count, &value->dnlnk_data_svc_info[value->dnlnk_data_svc_info_count++]);
      }  else
            count += sizeof(pfcp_ie_header_t) + ntohs(ie_header->len);
      }
      return value->header.len + PFCP_IE_HEADER_SIZE;
...
//	buf_len = sizeof(*value) - 4;
	...
	while (count < buf_len) {
		...
		if (ie_type == PFCP_IE_SRC_INTFC) {
			...
			count += decode_pfcp_3gpp_intfc_type_ie_t(buf + count, &value->src_intrc_type);
		}  else {
			count += sizeof(pfcp_ie_header_t) + ntohs(ie_header->len);
		}
	}
      ...
      pfcp_sess_estab_rsp_t *value)
{
      ...
      value->created_pdr_count = 0;
      value->created_traffic_endpt_count = 0;
      ...
      if (ie_type == PFCP_IE_NODE_ID) {
            ...
            count += decode_pfcp_created_pdr_ie_t(buf + count, &value->created_pdr[value->created_pdr_count++]);
      }  else if (ie_type == IE_LOAD_CTL_INFO) {
            count += decode_pfcp_load_ctl_info_ie_t(buf + count, &value->load_ctl_info);
      }  else if (ie_type == IE_OVRLD_CTL_INFO) {
            count += decode_pfcp_ovrld_ctl_info_ie_t(buf + count, &value->ovrld_ctl_info);
            ...
            count += decode_pfcp_created_traffic_endpt_ie_t(buf + count, &value->created_traffic_endpt[value->created_traffic_endpt_count++]);
      }  else
            count += sizeof(pfcp_ie_header_t) + ntohs(ie_header->len);
      }
      return count + ((value->header.s == 1) ? sizeof(pfcp_header_t) : (sizeof(pfcp_header_t) - 8));
